    ...
    for (auto [freq, positions] : antennas) {
        ...
        for (int i=0; i<positions.size(); ++i) {
            for (int j=i+1; j<positions.size(); ++j) {
                ...
                pair<int, int> antinode1 = {top.first, top.second};
                pair<int, int> antinode2 = {bottom.first, bottom.second};

                while (inBounds(antinode1)) {
                    grid[antinode1.first][antinode1.second] = '#';

                    antinode1.first -= xDelta;
                    antinode1.second -= yDelta;
                }

                while (inBounds(antinode2)) {
                    grid[antinode2.first][antinode2.second] = '#';

                    antinode2.first += xDelta;
                    antinode2.second += yDelta;
                }                
            }
        }
    }
